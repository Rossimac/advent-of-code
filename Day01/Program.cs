...
foreach (var line in lines)
{
    ...
    var lastDigit = GetFirstDigit(line, true);

    var digit = firstDigit * 10 + lastDigit;

    sum += digit;
    continue;

    int GetFirstDigit(IEnumerable<char> s, bool reverse)
    { 
        ...
        var growingInput = string.Empty;

        var directionalInput = reverse ? s.Reverse() : s;

        foreach (var character in directionalInput)
        {
            if (int.TryParse(character.ToString(), out var val))
            {
                return val;
            }

            growingInput += character;

            for (var index = 0; index < numbers.Count; index++)
            {
                var number = numbers[index];
                
                var directionalNumber = reverse ? number.Reverse() : number;
                if (growingInput.ToLowerInvariant().Contains(string.Join("", directionalNumber)))
                {
                    return index + 1;
                }
            }
